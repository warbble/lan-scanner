...
export class NetworkScanner extends EventEmitter {
  ...
  async scanNetwork(options: ScanOptions = {}): Promise<ScanResult> {
    ...
    try {
      ...
      // Enrich with MAC addresses, hostnames, device detection, and ports
      const limit = createLimiter(6); // Reduced concurrency for port scanning
      const enrichmentTasks = activeHosts.map(device => 
        limit(async () => {
          try {
            ...
            // Detect device type from MAC vendor
            if (device.mac) {
              const deviceInfo = detectDeviceType(device.mac);
              if (deviceInfo.vendor) device.vendor = deviceInfo.vendor;
            ...
            if (this.config.enablePortScan !== false) {
              const ports = await scanPorts(device.ip);
              if (ports.length > 0) device.ports = ports;
            }
